...
static void eventcb(struct bufferevent *bev, short what, void *ctx)
{
	...
	if (what & (BEV_EVENT_EOF|BEV_EVENT_ERROR))
	{
		...
		if (partner)
		{
			...
			{
				/* We still have to flush data from the other side, but when that's done, close the other side. */
				bufferevent_setcb(partner, NULL, close_on_finished_writecb, eventcb, NULL);
				bufferevent_disable(partner, EV_READ);
			}
			else
			{
				/* We have nothing left to say to the other side; close it. */
...
static void accept_cb(struct evconnlistener *listener, evutil_socket_t fd, struct sockaddr *a, int slen, void *p)
{
...
int main(int argc, char **argv)
{
	...
	struct evconnlistener *listener = evconnlistener_new_bind(base, accept_cb, NULL, LEV_OPT_CLOSE_ON_FREE|LEV_OPT_CLOSE_ON_EXEC|LEV_OPT_REUSEABLE, -1, (struct sockaddr*)&listen_on_addr, socklen);
